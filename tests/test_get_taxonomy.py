...
class TestGetTaxonomy(unittest.TestCase):
    ...
    def test_ebird_taxonomy(self):
        ...
        with mock.patch('photo_id.get_taxonomy.os.path.isfile') as mock_isfile, \
            mock.patch("builtins.open", mock.mock_open(read_data=json.dumps(test_json))) as mock_file, \
            mock.patch('photo_id.get_taxonomy.get_taxonomy') as mock_get_taxonomy:
            mock_isfile.return_value = True
            mock_get_taxonomy.return_value=test_json
            taxonomy = photo_id.get_taxonomy.ebird_taxonomy()
            mock_file.assert_called_with(".cache/taxonomy", encoding='utf-8', mode='rt')
        ...
        with mock.patch('photo_id.get_taxonomy.os.path.isfile') as mock_isfile, \
            ...
            mock_get_taxonomy.return_value=test_json
            taxonomy = photo_id.get_taxonomy.ebird_taxonomy()
            mock_file.assert_called_with(".cache/taxonomy", encoding='utf-8', mode='wt')
            self.assertEqual(taxonomy, test_json)
