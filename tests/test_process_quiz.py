...
class TestSortQuiz(unittest.TestCase):
    ...
    def test_sort_quiz_correct_order(
        self, mock_exists, mock_json_load, mock_open
    ):
        # Setup mock behavior
...
class TestBuildQuizFromTargetSpecies(TestCase):
    ...
    @mock.patch("json.dump")
    @mock.patch("os.path.exists", return_value=True)
    def test_with_empty_input_file(
        self, _mock_exists, _mock_json_dump, mock_open
    ):
        build_quiz_from_target_species(
            "input.txt", 3, "output.json", 5, 6, "LC"
        )
...
class TestSplitQuiz(unittest.TestCase):
    ...
    def test_split_quiz_single_file(
        self,
        mock_sorted_species,
        mock_process_quiz_file,
        mock_open,
        mock_json_dump,
    ):
        """
        ...
        self.assertEqual(
            mock_open.mock_calls[0][1][0].lower(),
            f"{os.getcwd()}/input_file_Part1.json".lower(),
        )
        mock_open.assert_called_with(mock.ANY, "wt", encoding="utf-8")
    ...
    @mock.patch("photo_id.process_quiz.process_quiz_file")
    @mock.patch("photo_id.process_quiz.sorted_species")
    def test_split_quiz_multiple_files(
        self,
        mock_sorted_species,
        mock_process_quiz_file,
        mock_open,
        ...
        # Setup
        taxonomy = [
            {"comName": "Species A", "taxonOrder": 1},
    ...
    @mock.patch("photo_id.process_quiz.process_quiz_file")
    @mock.patch("photo_id.process_quiz.sorted_species")
    def test_split_quiz_empty_species_list(
        self,
        mock_sorted_species,
        mock_process_quiz_file,
        mock_open,
    ...
    @mock.patch("photo_id.process_quiz.process_quiz_file")
    @mock.patch("photo_id.process_quiz.sorted_species")
    def test_split_quiz_file_naming(
        self,
        mock_sorted_species,
        mock_process_quiz_file,
        mock_open,
        ...
        taxonomy = [
            {"comName": "Species A", "taxonOrder": 1},
            {"comName": "Species B", "taxonOrder": 2},
            {"comName": "Species C", "taxonOrder": 3},
        ]
        quiz_data = {
            "location": "Test Location",
            "start_month": 1,
        ...
        self.assertEqual(
            mock_open.mock_calls[3][1][0].lower(),
            f"{os.getcwd()}/input_file_Part2.json".lower(),
        )
        self.assertEqual(
            mock_open.mock_calls[6][1][0].lower(),
...
class TestApplyAvonetData(unittest.TestCase):
    ...
    @mock.patch(
        "photo_id.process_quiz.json.load", side_effect=FileNotFoundError
    )
    ...
    def test_apply_avonet_data_successful_application(
        ...
        mock_json_load.return_value = {
            "species": [
                {"comName": "Brambling", "sciName": "Fringilla montifringilla"}
            ]
        }
        ...
        mock_open.assert_any_call(
            "valid_file.json", encoding="utf-8", mode="rt"
        )
        mock_open.assert_any_call(
            "valid_file.json", encoding="utf-8", mode="wt"
        )
